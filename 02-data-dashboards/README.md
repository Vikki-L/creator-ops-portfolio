# 02 · Data Dashboards
## 数据看板（对内运营 + 对外竞品 + 决策辅助）

> 6 个数据看板：内部运营监控 + 竞品情报 + 大使潜力评分 · 把决策周期从 2 周缩短到每天。

## Overview

这一系列看板的核心理念：让 Coach 团队和管理层**每天打开就能看到下属今天/昨天的真实表现**，而不是周末翻 Slack + 拉 Excel 才能拼出一份数据。

覆盖三个数据维度：
- **对内运营**：B1 Coach Daily Monitor · B5 社媒 QC 平台
- **对外情报**：B2 竞品账号监控 · B4 Template 数据看板
- **决策辅助**：B3 校园大使潜力评分（自动评级 · 辅助清退判断）
- **数据流转**：B6 Creator 投稿入库（数据源头）

## Modules

| 模块 | 一句话 | 状态 |
|------|--------|------|
| [B1 · Coach Daily Monitor](./B1-coach-daily-monitor/) | 达人每日表现实时大屏 · 决策周期 2 周 → 每天 | ✅ 已上线 |
| [B2 · Competitor Monitor](./B2-competitor-monitor/) | 31 字段竞品账号监控中台 · 覆盖 5-8 品牌 1000+ 账号 | ✅ 已上线 |
| [B3 · Ambassador Scoring](./B3-ambassador-scoring/) | Python 自研评分模型 · 评估 300+ 辅助清退 ~100 | ⭐ 核心案例 |
| [B4 · Template Dashboard](./B4-template-dashboard/) | 模板维度的爆款率 / 命中率统计 | ✅ 已上线 |
| [B5 · Content QC Platform](./B5-content-qc-platform/) | 社媒内容质量监控 · 三分结构总览 | ✅ 已规划 |
| [B6 · Creator Submission Pipeline](./B6-creator-submission-pipeline/) | Slack 投稿自动入库 → 结算/审核 | ✅ 已上线 |

## Key Outcomes

- 决策周期 **2 周 → 每天**
- 评估 **300+ 校园大使**，辅助清退 **~100 位**低效大使
- 监控 **1000+ 竞品账号** across 5-8 品牌
- 投稿数据采集去人工化

---

[← Back to Main Portfolio](../)
